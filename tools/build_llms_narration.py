...
def generate(job, folder=None):
    lang, entry = job
    digest = hashlib.sha256(('speech-2.8-hd'+voices[lang]+entry['text']).encode()).hexdigest()[:12]
    filename = f'llms-{lang}-{entry["id"]}-{digest}.mp3'
    target = (folder or ROOT/'dist/audio')/filename
    target.parent.mkdir(parents=True, exist_ok=True)
    script = ROOT/'narration/scripts'/f'{filename}.txt'
    script.parent.mkdir(parents=True, exist_ok=True)
    script.write_text(entry['text'], encoding='utf-8')
    if not target.exists() and entry.get('segments'):
        ffmpeg=shutil.which('ffmpeg') or str(Path.home()/'ffmpeg/bin/ffmpeg.exe')
        cache=ROOT/'narration/.audio-cache'
        cues=[]
        with tempfile.TemporaryDirectory() as tmp:
            chunks=[]; cursor=0
            for i, text in enumerate(entry['segments']):
                _, part=generate((lang,dict(id=entry['id']+f'-part{i}',text=text)),cache)
                wav=Path(tmp)/f'{i}.wav'
                subprocess.run([ffmpeg,'-v','error','-i',str(cache/part['file']),'-ar','32000','-ac','1',str(wav)],check=True)
                with wave.open(str(wav),'rb') as reader:
                    duration=reader.getnframes()/reader.getframerate()
                    chunks.append(reader.readframes(reader.getnframes()))
                cues.append(dict(start=cursor,end=cursor+duration));cursor+=duration
            combined=Path(tmp)/'combined.wav'
            with wave.open(str(combined),'wb') as writer:
                writer.setnchannels(1);writer.setsampwidth(2);writer.setframerate(32000);writer.writeframes(b''.join(chunks))
            subprocess.run([ffmpeg,'-v','error','-i',str(combined),'-b:a','128k',str(target)],check=True)
        target.with_suffix('.json').write_text(json.dumps(dict(text=entry['text'],duration_ms=cursor*1000,cues=cues)),encoding='utf-8')
    ...
    if target.with_suffix('.json').exists():
        ...
        meta={'text':previous['text'],'duration_ms':previous['duration']*1000,'cues':previous.get('cues')}
    assert meta['text']==entry['text'] and target.stat().st_size>1024
    item=dict(entry, file=filename, duration=meta['duration_ms']/1000)
    if meta.get('cues'):item['cues']=meta['cues']
    return lang, item
