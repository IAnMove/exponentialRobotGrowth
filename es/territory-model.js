...
export const TOWNS=[
 ...
 {name:'Ciudad del valle',tasks:[24,36,30,24,30,36],travel:5}
];
export const townCapacity=t=>t.tasks.map((n,i)=>Math.floor(n*SECTORS[i].limits[1]));
...
export function tickTerritory(s){
 ...
 if(!s.urbanHistory.length||s.time-s.urbanHistory.at(-1).time>=1-1e-8||s.time>=END)
