...
// Deep gradient sky, a blueprint floor that fades around a moving centre, and slow dust.
export function createAtmosphere(scene,{pointScale,dust=520,spread=[36,11,26]}={}){
  const res=[];
  const sky=new THREE.Mesh(new THREE.SphereGeometry(160,32,16),new THREE.ShaderMaterial({side:THREE.BackSide,depthWrite:false,fog:false,
    vertexShader:'varying vec3 vP;void main(){vP=normalize(position);vec4 p=projectionMatrix*modelViewMatrix*vec4(position,1.);gl_Position=p;}',
  ...
  sky.renderOrder=-10;scene.add(sky);res.push(sky.geometry,sky.material);
  const uniforms={uTime:{value:0},uCenter:{value:new THREE.Vector2()},uGlow:{value:new THREE.Color(0x0d1c3c)}};
  const floor=new THREE.Mesh(new THREE.PlaneGeometry(400,400),new THREE.ShaderMaterial({transparent:true,depthWrite:false,uniforms,
    vertexShader:'varying vec3 vW;void main(){vec4 w=modelMatrix*vec4(position,1.);vW=w.xyz;gl_Position=projectionMatrix*viewMatrix*w;}',
    fragmentShader:`uniform float uTime;uniform vec2 uCenter;uniform vec3 uGlow;varying vec3 vW;
float grid(vec2 p,float s){vec2 q=p/s;vec2 g=abs(fract(q-.5)-.5)/fwidth(q);return 1.-min(min(g.x,g.y),1.);}
void main(){float d=length(vW.xz-uCenter);float fade=exp(-d*d*.004);float minor=grid(vW.xz,.5)*.16,major=grid(vW.xz,2.5)*.42;
...
gl_FragColor=vec4(line+uGlow*pool*.6,(minor+major+ring)*fade+pool*.45);}`}));
  floor.rotation.x=-Math.PI/2;floor.position.y=-.02;scene.add(floor);res.push(floor.geometry,floor.material);
  const seed=Array.from({length:dust},(_,i)=>{const r=Math.sin(i*12.9898)*43758.5453;return r-Math.floor(r);}),geometry=new THREE.BufferGeometry();
  geometry.setAttribute('position',new THREE.BufferAttribute(new Float32Array(dust*3),3));
  geometry.setAttribute('aSize',new THREE.BufferAttribute(Float32Array.from(seed,s=>.04+s*.09),1));
  geometry.setAttribute('aAlpha',new THREE.BufferAttribute(Float32Array.from(seed,s=>.12+(1-s)*.35),1));
  const points=new THREE.Points(geometry,pointMaterial(pointScale,0x7fa9ff,1.1));points.frustumCulled=false;scene.add(points);res.push(geometry,points.material);
  const centre=new THREE.Vector3();
  return {update(time,center=null){uniforms.uTime.value=time;if(center){uniforms.uCenter.value.set(center.x,center.z);centre.set(center.x,0,center.z);}
...
export function createStageKit({es=false,reduce=false,pointScale={value:400},light=null,labelFog=false}={}){
  ...
  function text(str,{size=.32,color=CSS.ink,weight=650,mono=false,pill=null,stroke=null,align='center'}={}){
    ...
    const sprite=new THREE.Sprite(own(new THREE.SpriteMaterial({map:texture,transparent:true,depthTest:false,depthWrite:false,toneMapped:false,fog:labelFog})));
  ...
  function tensor(data,{x=0,y=1.8,z=0,w=2.4,h=3,title='',color=C.blue,selected=-1,probability=false,future=false,depth=.5,reveal=null,rowInfo=null,dimOthers=false,showDims=true,titleColor=CSS.ink}={}){
    ...
    const update=({sig,live,clock,R})=>{
      for(let k=0;k<n;k++){
        ...
        if(kit.hovered?.mesh===mesh&&kit.hovered.id===k)g+=.9;
        glow.setX(k,g);
      }
  ...
  function contextGrid(run,{x=3.6,y=2.4,z=0,cols=8,size=.34,highlight=null,fill=[.02,.3]}={}){
    ...
    S.userData.ticks.push(({R})=>{
      ...
      for(let k=0;k<N;k++){const e=back(clamp01(f-k)),p=cellPos(k);p.z=size/2;s.setScalar(size*Math.max(.001,e));m4.compose(p,q,s);mesh.setMatrixAt(k,m4);
        const fresh=clamp01(1-(f-k)/4);glow.setX(k,(highlight!==null&&highlight===owners[k]?.45+.25*Math.sin(time*3+k*.4):.08)+fresh*1.4*(e>0?1:0)+(kit.hovered?.mesh===mesh&&kit.hovered.id===k?.9:0));}
      mesh.instanceMatrix.needsUpdate=true;glow.needsUpdate=true;
    });
  ...
  function replay(set){set.userData.clock=0;for(const a of set.userData.anims)a.done=false;}
  function leave(set){set.userData.leaving=0;set.userData.leaveMats=collect(set);}
