...
La portada está en `site-src/hub/`; su colección se define en `atlas.js`. Notebooks actuales: 01 Robots, 02 Terafab, 03 Esfera de Dyson, 04 Hogar, 05 Starlink, 06 SpaceX, 07 Kardashev, 08 LLMs, 09 Mente, 10 Modelos. El museo (`/museo/`) es la visita a pie: cada experiencia está en una pantalla, se puede entrar en ella o abrirla como página. Modelos explica las familias que faltaban y la factura de entrada, caché y salida. `node check-mente.mjs`, `node check-modelos.mjs` y `node check-museo.mjs` cubren esos modelos. Para añadir otra explicación: crear su carpeta fuente, registrarla en la colección y añadirla al bucle de carpetas independientes en `tools/build_site.py`. Los recursos compartidos de Robots siguen en la raíz de `dist/`; no se duplican los audios ni Three.js.

`/starlink/` y `/spacex/` son notebooks 3D. Starlink muestra capas orbitales (53°, 43°, 70°, polar), un usuario móvil, pasarelas y el paquete con o sin láser; el recuento 3D es un muestreo, la flota real (~11.118 activos, 13 sept 2026) va en el tablero. SpaceX anima Falcon 9 (aterrizaje en barcaza) y Starship (captura en torre); los tiempos son típicos y el despliegue va comprimido. `node check-starlink.mjs` y `node check-spacex.mjs` cubren luz LEO vs GEO, cobertura y la línea de tiempo.

`/dyson/` y `/es/dyson/` son un modelo 3D del enjambre que Freeman Dyson describió en 1960, comparado con la cáscara rígida de ciencia ficción. El modelo físico (`model.js`) conserva la luminosidad solar: lo interceptado más lo que sigue escapando suma 3,828×10²⁶ W. La temperatura del enjambre es la de una placa de dos caras y no depende de la cobertura; la de cáscara cerrada solo se aplica al 100 %. La materia usa la masa de Júpiter como orden de magnitud, con densidad 3000 kg/m³ declarada. El Sol y la Tierra de la escena no están a escala. `node check-dyson.mjs` comprueba constante solar, conservación, T ∝ R⁻½ y la comparación con ~19 TW de consumo mundial (600 EJ en 2025).

Construir con `python build-site.py`. Validar con `python check-site.py`, `node check-terafab.mjs`, `node check-dyson.mjs`, `node check-starlink.mjs`, `node check-spacex.mjs`, `node check-explanations.mjs`, `node check-mente.mjs`, `node check-localization.mjs` y `node check-narrator.mjs`. Las comprobaciones pasan; no se realizó QA visual en navegador.
