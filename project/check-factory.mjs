...
function conserved(s){assert(s.deployed<=s.total);assert.equal(s.delivered,s.total+s.queues.reduce((a,b)=>a+b,0)+s.jobs.filter(x=>x!==null).length);for(let i=1;i<5;i++)assert(s.queues[i]>=0&&s.queues[i]<=BUFFER);s.queues.forEach(n=>assert(Number.isInteger(n)));assert(s.jobs.every(p=>p===null||(p>=0&&p<=1)));}
...
const s=createFactory();assert.equal(automate(s,1),false,'A robot must be manufactured before deployment');advance(s,1);assert(automate(s,1));assert.equal(capacity(s,1).robots,0);assert.equal(robotDuty(s,1,0),'arriving');assert.equal(automate(s,1),true);assert.equal(automate(s,1),false);
advance(s,5);conserved(s);assert.equal(capacity(s,0).humans,2);advance(s,5);assert.equal(capacity(s,0).humans,0);
...
const automatic=createFactory(undefined,true);for(let k=0;k<24/STEP;k++){tick(automatic);conserved(automatic);assert.equal(automatic.robots.flat().length,automatic.deployed);}assert.equal(automatic.deployed,10);assert(automatic.total>human);assert(automatic.lastDeployment>0);
