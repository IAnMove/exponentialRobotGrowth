...
export const standPosition=index=>({x:-3,y:1.05,z:STOPS[index].z+2.7});
export const obstacles=[...STOPS.map(s=>({minX:-9.5,maxX:-3.3,minZ:s.z-5.2,maxZ:s.z+5.2})),...STOPS.map(s=>({minX:-3.35,maxX:-2.65,minZ:s.z+2.35,maxZ:s.z+3.05}))];
export function poseAt(index){return {x:.8,z:STOPS[index].z,yaw:Math.PI/2,pitch:.05};}
