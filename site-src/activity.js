...
export function createActivity(scene,world,industries){
  ...
  let lastStocks='';
  function update(frame,next,time,phase,selected,showRoutes,traceSources=[]){
    ...
    routes.forEach(r=>{const traced=traceSources.includes(r.from)&&selected===r.to;const visible=traced||showRoutes||selected===r.from||selected===r.to;r.line.visible=visible;const blocked=statuses[r.to].code==='waiting'||statuses[r.to].code==='supply';r.line.material.color.set(blocked?0xffad67:0x8edccc);r.line.material.opacity=traced?1:selected===r.from||selected===r.to?.9:.3;r.line.material.color.set(traced?0xffd17f:blocked?0xffad67:0x8edccc);r.arrows.forEach((o,j)=>{o.visible=visible&&statuses[r.from].rate>0;const p=routePoint(r.path,(time*.08+j/4)%1);o.position.set(p.x,.65,p.z);o.rotation.set(Math.PI/2,p.angle,0,'YXZ');});});
  }
  return {update};
}
