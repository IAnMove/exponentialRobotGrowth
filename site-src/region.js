...
try{world=createRegionWorld($('r-canvas'));$('r-loading').hidden=true;}catch(e){$('r-loading').textContent='No se pudo iniciar el mundo 3D. Los controles y la comparación siguen disponibles.';console.error(e);}
const labels=[],tabs=[];
...
function update(){
  ...
  $('r-fleet').textContent=fmt(state.fleet);$('r-fleet-note').textContent='24 iniciales + '+fmt(state.built)+' nuevos';$('r-buildings').textContent=n.reduce((a,b)=>a+b,0);$('r-projects').textContent=projects.length+' obras en marcha';$('r-rate').textContent=fmt(state.flow[3],1);$('r-multiple').textContent=fmt(reference.flow[3],1)+' / ciclo sin nuevas instalaciones';
  ...
  $('r-gain').textContent=fmt(state.fleet/reference.fleet,2)+'×';$('r-chart-end').textContent='Ciclo '+Math.floor(state.time)+' / 120';
  chart($('r-chart'),[{values:[[0,24],...reference.history.map(p=>[p.time,p.fleet])],color:'#839eaf'},{values:[[0,24],...state.history.map(p=>[p.time,p.fleet])],color:'#a7e0cf'}]);$('r-chart').setAttribute('aria-label','Flota: '+fmt(state.fleet)+'; sin nuevas instalaciones: '+fmt(reference.fleet));
...
function select(i,narrate=true){if(i<0)return;selected=i;$('r-inspector').hidden=false;world?.select(i);update();const p=state.sites[i];if(narrate)narrator.explain('region-'+p.type,p.status==='building'?'region-project':p.status==='open'?'region-operation':null);if(narrate&&matchMedia('(max-width:760px)').matches)$('r-inspector').scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'start'});}
...
$('r-build').onclick=()=>{if(startBuild(state,selected)){playing=true;last=performance.now();update();}};$('r-close').onclick=()=>{selected=-1;$('r-inspector').hidden=true;world?.select(-1,false);world?.fit();update();};$('r-enter').onclick=()=>location.href='./district.html';
