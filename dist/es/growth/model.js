...
export function simulate(p=defaults){
 let fleet=10,bank=0,pending=[],made=0,goods=0,humanTotal=0,fixedTotal=0;const rows=[];
 for(let day=0;day<=p.days;day++){
  ...
  const cap=p.limited?Math.min(p.slots*24,p.supply):Infinity;
  const human=Math.min(10*8*p.shifts,cap),fixed=Math.min(10*p.hours,cap),work=Math.min(fleet*p.hours,cap);
  const room=p.limited?Math.max(0,p.slots-fleet-pending.reduce((n,x)=>n+x.count,0)):Infinity;
  const buildWork=Math.min(work*p.reinvest,Math.max(0,room*p.buildHours-bank));
  const output=work-buildWork;
  rows.push({day,fleet,human,fixed,output,goods,humanTotal,fixedTotal,made,bank,pending:pending.reduce((n,x)=>n+x.count,0),work,buildWork,limited:work+1e-6<fleet*p.hours});
  if(day===p.days)break;
