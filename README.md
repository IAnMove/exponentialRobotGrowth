...
La portada está en `site-src/hub/`; su colección se define en `atlas.js`. Notebooks actuales: 01 Robots, 02 Terafab, 03 Esfera de Dyson, 04 Hogar, 05 Starlink, 06 SpaceX. Para añadir otra explicación: crear su carpeta fuente, registrarla en la colección y añadirla al bucle de carpetas independientes en `tools/build_site.py`. Los recursos compartidos de Robots siguen en la raíz de `dist/`; no se duplican los audios ni Three.js.
...
Construir con `python build-site.py`. Validar con `python check-site.py`, `node check-terafab.mjs`, `node check-dyson.mjs`, `node check-starlink.mjs`, `node check-spacex.mjs`, `node check-explanations.mjs`, `node check-localization.mjs` y `node check-narrator.mjs`. Las comprobaciones pasan; no se realizó QA visual en navegador.
